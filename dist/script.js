...
window.addEventListener('DOMContentLoaded', function () {
  // глобальный слайдер который состоит из 6 страниц в index.html
  var slider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["default"]({
    container: '.page',
    btns: '.next'
  });
  slider.render(); // глобальный слайдер который состоит из 8 страниц в module.html
  ...
  modulePageSlider.render(); // мини слайдер на странице 1 index.htmldex
  ...
  showUpSlider.init(); // мини слайдер на странице 3 index.html
  ...
  modulesSlider.init(); // мини слайдер на странице 5 index.html
  ...
  feedSlider.init(); // плеер на 1 странице index.html
...
function () {
  ...
  } // навешиваем функционал на плюсы
  ...
  _createClass(Difference, [{
    ...
    value: function bindTriggers() {
      ...
      this.oldOfficer.querySelector('.plus').addEventListener('click', function () {
        ...
        if (_this.oldCounter !== _this.oldItems.length - 2) {
          ...
          // когда доходим до последней карточки, блок с плюсом скрываем
          _this.oldItems[_this.oldCounter].style.display = 'flex';
    ...
    } // скрываем все карточки кроме последней, которая является кнопкой плюс
    ...
    value: function init() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          e.preventDefault(); // после сабмита будет скрыватся форма и будет показыватся статус отправки c задаными стилями

          var statusMessage = document.createElement('div');
          statusMessage.style.cssText = "\n                margin-top: 15px;\n                font-size: 18px;\n                color: orange;\n                ";
          item.parentNode.appendChild(statusMessage);
          statusMessage.textContent = _this.message.loading; // формируем данные и отправляем их на сервер
  ...
  _createClass(VideoPlayer, [{
      ...
      this.btns.forEach(function (btn, i) {
        ...
        btn.addEventListener('click', function () {
          ...
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            _this.activeBtn = btn; // условие что б не создавать новый плеер (createPlayer)

            if (document.querySelector('iframe#frame')) {
              _this.overlay.style.display = 'flex'; // условие что бы не создавать каждый раз новый плеер, если пользователь кликает на тоже самое видео
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; // копируем svg иконку со всеми параметрами, что б в будущем заменить замочек в неактивном блоке

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // прописываем условия, когда пользоватеь досмотрел видео до конца(state.data ===0)
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      } catch (error) {} // скрываем все слайды и оставляем только один для отображения на странице
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          _this2.slides[_this2.slideIndex - 1].classList.remove('slideInLeft', 'slideInRight');
        ...
        btn.parentNode.previousElementSibling.addEventListener('click', function (e) {
          ...
          _this2.slides[_this2.slideIndex - 1].classList.remove('slideInUp', 'slideInLeft', 'slideInRight');
      ...
      this.prevModule.forEach(function (item) {
        item.addEventListener('click', function (e) {
          ...
          _this2.slides[_this2.slideIndex - 1].classList.remove('fadeIn', 'slideInUp', 'slideInRight');
      ...
      this.nextModule.forEach(function (item) {
        item.addEventListener('click', function (e) {
          ...
          _this2.slides[_this2.slideIndex - 1].classList.remove('fadeIn', 'slideInUp', 'slideInLeft');
...
function (_Slider) {
  ...
  } // убираем клас активности со всех слайдов кроме первого + стилизация 
  ...
  _createClass(MiniSlider, [{
    ...
    value: function nextSlide() {
      // отсеиваем кнопки, которые находятся в слайдере
    ...
    } // функционал на кнопки next prev
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          if (_this2.slides[i].tagName !== "BUTTON") {
            var active = _this2.slides[i]; // последний слайд в списке

            _this2.container.insertBefore(active, _this2.slides[0]); // ставим последний слайд на первую позицию
...
var Slider = // слайдер делаем на две страници
